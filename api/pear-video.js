...
export default function handler(req, res) {
    ...
    const output = path.join(process.cwd(), `pear-output-${id}.mp4`)
    const imagePath = path.join(process.cwd(), `pear-img-${id}.png`)
    const moodPath = path.join(process.cwd(), `pear-mood-${id}.txt`)
